...
def get_data_fixed_batt(batt):
    ...
    path = path = '/Users/jacquelinedowling/MEM_Nov2019/SEM-1.2/Output_Data/Oct29_syscont'
...
for i in range(0, len(values)):
    if values[i] == 0:
        ...
        times_cheaper.append(round((1 / values[i]),1))
print(values)
print(times_cheaper)
#1/10
#Out[193]: 0.1
...
#    name = int(times_cheaper[toplot[i]])
    ...
    ax1.set_title(str(name)+ 'x cheaper LDS')
#    ax1.set_xlim(dates[0], dates[-8600])
#    q = 120
    ...
    q = 600
    ax1.set_ylim(top= q, bottom=0)
    ax2.set_ylim(top= q/24, bottom=0)
...
plt.savefig('si/SI_niches_lds.pdf', bbox_inches='tight')
#plt.savefig('SI_niches_pgp.eps', bbox_inches='tight')
plt.show()
