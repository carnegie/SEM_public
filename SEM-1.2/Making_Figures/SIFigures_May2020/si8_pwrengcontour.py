...
# -*- coding: utf-8 -*-
"""
Created on Mon Jan  6 14:35:18 2020
...
#        if re.match("PGPtest5_systemsensitivity_pgp"+str(pgp)+"batt\d+.pickle", filename):
#            num = filename[-11:-7]
#            nums.append(int(num))
...
#        pickle_in = open(str(path)+ "/PGPtest5_systemsensitivity_pgp"+str(pgp)+"batt"+ str(factors[i])+".pickle","rb")
...
def get_data_fixed_batt_power(batt):
    nums = []
    factors = []
    costs = []
    
    
    path = '/Users/jacquelinedowling/MEM_Nov2019/SEM-1.2/Output_Data/April2020_powercont'
#    path = "/Users/jacquelinedowling/SEM-1.1/Output_Data/PGPtest5_systemsensitivity"
    for filename in os.listdir(path):
#        PGPtest5_systemsensitivity_pgp0200batt0300.pickle
        if re.match("April2020_powercont_pgp\d+batt"+str(batt)+".pickle", filename):
            num = filename[-19:-15]
            nums.append(int(num))
#    print('before sort',nums)
    nums.sort()
...
#    print('after sort',factors)
    ...
    for i in range(0,len(factors)):
        pickle_in = open(str(path)+ "/April2020_powercont_pgp"+ str(factors[i])+"batt"+str(batt)+".pickle","rb")
        base = pickle.load(pickle_in)
        ...
        if results['SYSTEM_COST'] == -1:
            costs.append(None)
            print('Failed to solve:',"pgpp"+ str(factors[i])+"battp"+str(batt))
        else:
            costs.append(results['SYSTEM_COST'])
...
def get_data_fixed_batt_energy(batt):
    nums = []
    factors = []
    costs = []
    
    path = '/Users/jacquelinedowling/MEM_Nov2019/SEM-1.2/Output_Data/April2020_energycont'
#    path = "/Users/jacquelinedowling/SEM-1.1/Output_Data/PGPtest5_systemsensitivity"
    for filename in os.listdir(path):
#        PGPtest5_systemsensitivity_pgp0200batt0300.pickle
        if re.match("April2020_energycont_pgp\d+batt"+str(batt)+".pickle", filename):
            num = filename[-19:-15]
            nums.append(int(num))
#    print('before sort',nums)
    nums.sort()
...
#    print('after sort',factors)
    ...
    for i in range(0,len(factors)):
        pickle_in = open(str(path)+ "/April2020_energycont_pgp"+ str(factors[i])+"batt"+str(batt)+".pickle","rb")
        base = pickle.load(pickle_in)
        ...
        if results['SYSTEM_COST'] == -1:
            costs.append(None)
            print('Failed to solve:',"pgpe"+ str(factors[i])+"batte"+str(batt))
        else:
            costs.append(results['SYSTEM_COST'])
...
battlistp = []
for i in range(0, len(strings)):
    data = get_data_fixed_batt_power(strings[i])
    battlistp.append(data)

battliste = []
for i in range(0, len(strings)):
    data = get_data_fixed_batt_energy(strings[i])
    battliste.append(data)
...
ax2 = plt.figure()
#0.06, 0.065,
levels = [0.065, 0.07,0.075, 0.08, 0.085, 0.09, 0.095, 0.1, 0.105, 0.11, 0.115, 0.12]
...
q = plt.clabel(CS2, inline=1, fontsize=10, fmt=fmt, rightside_up = True)

q[0].set_rotation(270)
...
plt.xlabel('LDS power cost\n(multiple of base case)')
plt.ylabel('Battery cost\n(multiple of base case)')

# Make a colorbar for the ContourSet returned by the contourf call.
cbar = plt.colorbar(CS, shrink=0.8, extend='both',format='%.2f', ticks=[0.07, 0.08, 0.09, 0.10, 0.11, 0.12])
...
#ax2.scatter(0.144675926, 1, marker="o", color='white', s=50)
#ax2.text(1.28, .08, "PGP\nwith H2\nturbines", color='white', fontsize=12, fontweight='bold')


plt.savefig('si/SI_contourpower.pdf', bbox_inches='tight')
plt.show()
...
CS = plt.contourf(X, Y, Z, levels, cmap = plt.set_cmap('viridis_r'))
CS2 = plt.contour(CS, levels=[0.08, 0.09,0.1,0.11,0.12], colors='k')
#CS2 = plt.contour(CS, levels=CS.levels[::2], colors='k')
fmt = ticker.FormatStrFormatter('$%.2f')
...
cbar.add_lines(CS2)
plt.savefig('si/SI_contourenergy.pdf', bbox_inches='tight')
plt.show()